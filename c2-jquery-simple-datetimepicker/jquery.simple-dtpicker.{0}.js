...
(function ($) {
    ...
    PickerHandler.prototype._relocate = function () {
        ...
        if ($input != null && $picker.data("isInline") === false) {
            ...
            if (
                $(".datepicker_calendar", $picker).width() !== 0 &&
                $(".datepicker_timelist", $picker).width() !== 0
            ) {
                $picker
                    .parent()
                    .width(
                        $(".datepicker_calendar", $picker).width() +
                        $(".datepicker_timelist", $picker).width() +
                        6
                    );
            }
            if (
                ...
                $picker
                    .parent()
                    .css("top", $input.offset().top + input_outer_height + "px");
            }
            ...
            if ($picker.parent().width() + $input.offset().left > $(window).width()) {
                ...
                $picker
                    ...
                    .css(
                        "left",
                        ($(window).width() - $picker.parent().width()) / 2 + "px"
                    );
            } else {
    ...
    var draw = function ($picker, option, year, month, day, hour, min) {
        ...
        if (oldDate != null) {
            if (
                ...
                oldDate.getHours() != date.getHours() ||
                oldDate.getMinutes() != date.getMinutes()
            ) {
                if (
                    date.getMinutes() === 0 ||
                    date.getMinutes() % minuteInterval === 0
                ) {
        ...
        for (var zz = 0; i < cellNum; i++) {
            ...
            if (
                ...
                $td.click(function (ev) {
                    ...
                    if (
                        $picker.data("dateOnly") === true &&
                        $picker.data("isInline") === false &&
                        $picker.data("closeOnSelected")
                    ) {
                        // Close a picker
                        ActivePickerId = -1;
                        $picker.hide();
        ...
        if ($picker.data("timeOnly") === true) {
            ...
            if ($link_next_month != null) $link_next_month.css("display", "none");
            if ($link_before_month != null) $link_before_month.css("display", "none");
        }
        if ($picker.data("dateOnly") === true) {
            ...
            $timelist.css(
                "height",
                Math.max($calendar.innerHeight() - 10, 200) + "px"
            );
            /* Output time cells */
            var hour_ = minTime[0];
            var min_ = minTime[1];
            while (hour_ * 100 + min_ < maxTime[0] * 100 + maxTime[1]) {
                ...
                var is_past_time =
                    ...
                    (hour_ == todayDate.getHours() && min_ < todayDate.getMinutes());
            ...
            if (isScroll === true) {
                ...
                $timelist.scrollTop(
                    timelist_activeTimeCell_offsetTop - $timelist.offset().top
                );
            } else {
    ...
    $(function () {
        $("body").click(function () {
            for (var i = 0; i < PickerObjects.length; i++) {
                ...
                if (
                    $picker.data("inputObjectId") != null &&
